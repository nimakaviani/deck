...
interface ICDK8sErrorDetails {
  error: 'missing_property' | 'invalid_type' | 'invalid_format' | 'invalid_value';
  message: string;
  pathExpression: string;
}

interface ICDK8sError {
  message: string;
  details?: ICDK8sErrorDetails;
}
...
function extractCustomError(error: ICDK8sError): { summary: string; debugDetails?: string } {
  if (!error) {
    return null;
  }
...
export function CDK8sExecutionDetails(props: IExecutionDetailsSectionProps) {
  const { stage } = props;
  const trigger = props.execution.trigger as IGitTrigger;
  const customError = extractCustomError(stage.context.error as ICDK8sError);
...
export namespace CDK8sExecutionDetails {
